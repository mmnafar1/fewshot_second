...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', help='cifar/mnist/fashionmnist/mvtec/paris', default='fundus')
    parser.add_argument('--input_dir', help='input image dir', default='Input/Images')
    parser.add_argument('--pos_class', help='normal class', default='good')
    parser.add_argument('--random_images_download', help='random selection of images', default=False)
    parser.add_argument('--num_images', type=int, help='number of images to train on', default=3)
    ...
    if opt.if_download == True:
        if dataset == 'cifar':
            ...
            opt.num_transforms, opt.niter = 54, opt.niter_rgb
            opt.input_name = download_class_fundus(opt)
        elif dataset == 'mnist':
    ...
    for i in range(num_images):
        real = img.imread("%s/%s_%d.jpg" % (opt.input_dir, opt.input_name[:-4], i))
