    ...
    while scale_num < opt.stop_scale+1:
        ...
        for epoch in range(opt.num_epochs):
            ...
            for id in index_image:
                plt.imsave('%s/real_scale%d.jpg' % (opt.outf[id], id), functions.convert_image_np(reals[id][scale_num]), vmin=0, vmax=1)
            if (nfc_prev==opt.nfc):
    ...
    for epoch in range(opt.niter):
        ...
        if index_transform == opt.num_transforms -1 and (epoch % 150 == 0 or epoch == (opt.niter - 1)):
            for j, id in enumerate(opt.outf):
                plt.imsave('%s/fake_sample_epoch%d.jpg' % (id,  epoch),
            ...
            for j, id in enumerate(opt.outf):
                plt.imsave('%s/G(z_opt)_epoch%d.jpg' % (id, epoch),
